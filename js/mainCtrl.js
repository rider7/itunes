...
$scope.getSongData = function() {
    itunesService.getArtist($scope.artist)
        ...
        var myFinalArray = [];
        var myResults = response.data.results;
        
        for(var i = 0; i < myResults.length; i++) {    
        var newObject = {
        AlbumArt: myResults[i].artworkUrl30,
        Artist: myResults[i].artistName,
        Collection: myResults[i].collectionName,
        CollectionPrice: myResults[i].collectionPrice,
        Play: myResults[i].previewUrl,
        Type: myResults[i].kind
            
        }
        myFinalArray.push(newObject)
}
